 ...
 * Most GDT either are Database enabled (GDT_String, GDT_Int, GDT_Enum) or mostly used for rendering like (GDT_Title, GDT_Link,
 * etc...)
 *
 * @author gizmore
 * @version 6.11.4
...
abstract class GDT
{
	...
	# String representation of current var
	private $valueConverted = false;

	public $value;

	# Object representation of current var
	public $initial;
	...
	# hide in tables, forms, lists and cards.
	public $notNull = false;

	# adds cascade when used with a primary key.
	...
	{
		Logger::log('gdt',
		sprintf('%d: %s', self::$COUNT, self::gdoClassNameS()));
		if (self::$DEBUG >= 2)
		{
			Logger::log('gdt',
			Debug::backtrace('Backtrace GDT allocation', false));
		}
	}
	...
	public function id()
	{
		return (GDT_Form::$CURRENT ? GDT_Form::$CURRENT->name . "_" : '') .
		str_replace([
			'[',
			']'
		], [
	...
	{
		return $this->name ? sprintf('id="%s"', $this->id()) : '';
	}

	public function htmlForID()
	{
	...
	public function error($key, array $args = null, $code = 409)
	{
		return $this->rawError(t($key, $args), $code);
	}

	public function rawError($html = null, $code = 409)
	{
		if ( !$this->error)
			$this->error = $html;
	...
	{
		return $this->error ? ('<div class="gdo-form-error">' . $this->error .
		'</div>') : '';
	}
	...
	# ## Table ###
	# ############
	public function gdtTable(GDO $table)
	{
		$this->gdtTable = $table;
		return $this;
	}
	...
	/**
	 * Set the var via value.
	 * Converted via toVar($value).
	 *
	 * @param mixed $value
	 * @return self
	 */
	...
	/**
	 * Set var and value in one step.
	 * Do not recompute value.
	 *
	 * @param string $var
	 * @param mixed $value
	 * @return self
	...
	{
		return ($var === null) || ($var === '') ? null : (string) $var;
	}
	...
	public function getParameterVar()
	{
		return $this->getRequestVar(null, $this->var);
	}

	public function getParameterValue()
	{
		return $this->toValue($this->getParameterVar());
	}
	...
	{
		$this->initial = $this->var = $var === null ? null : (string) $var;
		$this->valueConverted = false;
		return $this;
	}
	...
	/**
	 * Is this gdt automatically exchanged via systems, e.g.
	 * client<->server.
	 * Interesting for password hashes and secrets (not serialize). GDT_Password is serializable as it's just the password form
	 * field.
	 *
	 * @return boolean
	 */
	 ...
	 *       Document the weird quirk for GDT_AutoIncrement? (or where was the weird primary quirk?)
	 * @return boolean
	 */
	...
	{
		return GDT_Form::$CURRENT ? GDT_Form::$CURRENT->name : null;
	}
	...
	{
		return GDT_Form::$CURRENT ? sprintf('%s[%s]', $this->formVariable(),
		$this->name) : $this->name;
	}
	...
	{
		return sprintf(" name=\"%s\"", $this->formName());
	}
	...
	public function setGDOVar($var)
	{
		if ($this->gdo)
			$this->gdo->setVar($this->name, $var);
		return $this;
	}

	public function setGDOValue($value)
	{
		return $this->setGDOVar($this->toVar($value));
	}
	...
	/**
	 * Read a var from the $_REQUEST array.
	 * The tricky thing is to handle nested arrays.
	 *
	 * @TODO: _getRequestVar seems bulky and slow. Come up with better request args parsing.
	 * @param string $firstLevel
	 *        - root in $_REQUEST
	 * @param string $default
	 * @param string $name
	...
	{
		return Application::instance()->isCLI() ? $this->renderCLI() : $this->renderCell();
	}
	...
	public function renderCellSpan($var)
	{
		return sprintf('<span class="%s">%s</span>', $this->htmlClass(),
		html($var));
	}

	public function renderCard()
	{
		return sprintf('<label>%s</label><span>%s</span>', $this->displayLabel(),
		$this->displayVar($this->getVar()));
	}
	...
	{
		return is_object($choice) ? $choice->renderChoice() : $choice;
	}

	public $labelArgs;

	public function labelArgs(...$labelArgs)
	{
	...
	{
		return t($this->name, $this->labelArgs);
	}
	...
	{
		return $this->error('err_not_found');
	}
	...
	public function sort(array &$array, $ascending = true)
	{
		$_this = $this;
		uasort($array,
		function (GDO $a, GDO $b) use ($_this)
		{
	...
	public function orderDefaultAsc($defaultAsc = true)
	{
		$this->orderDefaultAsc = $defaultAsc;
		return $this;
	}

	public function orderDefaultDesc($defaultDesc = true)
	{
		$this->orderDefaultAsc = !$defaultDesc;
		return $this;
	}
	...
	{
		$ff = $this->filterField ? $this->filterField : $this->name;
		return $this->_getRequestVar("{$rq}[f]", null, $ff);
	}
	...
	/**
	 * Extend query with searching for a term.
	 * Used in quicksearch.
	...
	{
		$nameI = GDO::escapeIdentifierS(
		$this->searchField ? $this->searchField : $this->name);
		...
		{
			$st1 = GDO::escapeSearchS($searchTerm['min']);
			$st2 = GDO::escapeSearchS($searchTerm['max']);
			if ($st1 && $st2)
			{
				return sprintf('%s%s BETWEEN \'%s\' AND \'%s\'', $fkTable,
				$nameI, $st1, $st2);
			}
			if ($st1)
			{
		...
		switch ($searchTerm[0])
		{
			...
			default:
				...
				return sprintf('%s%s %s \'%%%s%%\'', $fkTable, $nameI, $op,
				$searchTerm);
			case '=':
				$op = '=';
				break;
		...
		return sprintf('%s%s %s \'%s\'', $fkTable, $nameI, $op, $searchTerm);
	}
	...
	# ## Config ###
	# #############
	public function displayConfigJSON()
	{
		return json_quote(json_encode($this->configJSON(), JSON_PRETTY_PRINT));
	}
